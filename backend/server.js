...
// Function to delete old global messages (older than 2 days)
async function deleteOldGlobalMessages() {
  try {
    const globalThreshold = new Date(Date.now() - 2 * 24 * 60 * 60 * 1000); // 2 days ago
    const deletedMessages = await Message.deleteMany({
      isPrivate: false,
      timestamp: { $lt: globalThreshold },
    });

    if (deletedMessages.deletedCount > 0) {
      console.log(`Deleted ${deletedMessages.deletedCount} global messages older than 2 days.`);
      io.emit("refreshMessages"); // Notify clients to refresh
    }
  } catch (err) {
    console.error("Error deleting old global messages:", err);
  }
}
...
mongoose
  ...
  .then(() => {
    ...
    deleteOldGlobalMessages(); // Run on startup
    // Run every 2 days (2 * 24 * 60 * 60 * 1000 milliseconds)
    setInterval(deleteOldGlobalMessages, 2 * 24 * 60 * 60 * 1000);
  })
  .catch((err) => console.error("MongoDB connection error:", err));
