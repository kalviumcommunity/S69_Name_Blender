...
// import { Send, Trash2, Edit2, LogOut, MoreVertical, Moon, Sun, ArrowLeft, Home } from "lucide-react";
...
//                       <span>{msg.text}</span>
//                       <div className={`text-xs ${darkMode ? "text-gray-400" : "text-gray-500"}`}>{formatTimestamp(msg.timestamp)}</div>
//                     </div>
...
//                 ))
//               )}
//               {typing && <div className="text-sm italic text-gray-400 animate-pulse">{recipientId} is typing...</div>}
...
```jsx
import React, { useState, useEffect, useRef } from "react";
import { Send, Trash2, Edit2, LogOut, MoreVertical, Moon, Sun, ArrowLeft, Home, Video } from "lucide-react";
import io from "socket.io-client";
import axios from "axios";
import { useParams, useNavigate } from "react-router-dom";
import { Tooltip } from "react-tooltip";
...
function PrivateChatPage() {
  ...
  const [error, setError] = useState(null);
  const [loading, setLoading] = useState(true);
  const [callStatus, setCallStatus] = useState(null); // null, "initiating", "incoming", "active", "ended"
  ...
  useEffect(() => {
    ...
    try {
      ...
      socket.on("call:incoming", ({ from }) => {
        ...
        setError(`Incoming call from ${from}`);
      });
      ...
      socket.on("call:rejected", () => {
        setCallStatus("ended");
        setError("Call rejected by recipient.");
        cleanupCall();
      });
      ...
      socket.on("call:offer", async ({ offer }) => {
        try {
          ...
          pc.onicecandidate = (event) => {
            if (event.candidate) {
              socket.emit("call:ice-candidate", { candidate: event.candidate, to: recipientId });
            }
          };
          ...
          socket.emit("call:answer", { answer, to: recipientId });
        } catch (err) {
      ...
      socket.on("call:answer", async ({ answer }) => {
        if (!peerConnection) {
          ...
          setError("Call setup failed: No peer connection.");
          return;
        }
        try {
      ...
      socket.on("call:ice-candidate", async ({ candidate }) => {
        if (!peerConnection || !candidate) {
          console.error("Invalid peer connection or candidate");
          return;
        }
        try {
  ...
  const initiateCall = async () => {
    try {
      ...
      socket.emit("call:initiate", { to: recipientId, offer, from: user.name });
  ...
  const acceptCall = async () => {
    try {
      ...
      socket.emit("call:accept", { to: recipientId });
      setCallStatus("active");
      setError(null);
    } catch (err) {
  ...
  const rejectCall = () => {
    socket.emit("call:reject", { to: recipientId });
    setCallStatus("ended");
    setError(null);
    cleanupCall();
  };
          ...
          <>
            {error && (
              <div
                className={`p-4 rounded-lg mb-4 ${
                  darkMode ? "bg-red-800 text-red-300" : "bg-red-100 text-red-700"
                }`}
              >
                {error}
              </div>
            ...
            <div className="flex justify-end mb-2">
              <button
                ...
                className={`p-2 rounded-full ${darkMode ? "bg-purple-700 hover:bg-purple-600 text-white" : "bg-purple-500 hover:bg-purple-400 text-white"} transition-all`}
              ...
              <div className={`p-4 rounded-lg ${darkMode ? "bg-gray-800" : "bg-gray-100"} mb-4`}>
                <div className="flex justify-between items-center mb-2">
                  <span className={darkMode ? "text-gray-300" : "text-gray-700"}>
                    ...
                    {callStatus === "incoming" && `Incoming call from ${recipientId}`}
                  ...
                  {callStatus === "incoming" && (
                    <div className="flex gap-2">
                      <button
                        onClick={acceptCall}
                        className={`px-3 py-1 rounded-full ${darkMode ? "bg-green-600 hover:bg-green-500" : "bg-green-400 hover:bg-green-300"} text-white`}
                      >
                        Accept
                      </button>
                      <button
                        onClick={rejectCall}
                        className={`px-3 py-1 rounded-full ${darkMode ? "bg-red-600 hover:bg-red-500" : "bg-red-400 hover:bg-red-300"} text-white`}
                      >
                        Reject
                      </button>
                    </div>
                ...
                {(callStatus === "active" || callStatus === "initiating") && (
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <p className={darkMode ? "text-gray-400" : "text-gray-600"}>You</p>
                      {localStream ? (
                        <video ref={localVideoRef} autoPlay muted className="w-full rounded-lg" />
                      ) : (
                        <div className={`w-full h-32 bg-gray-700 rounded-lg flex items-center justify-center ${darkMode ? "text-gray-400" : "text-gray-600"}`}>
                          Waiting for local video...
                        </div>
                      )}
                    </div>
                    <div>
                      <p className={darkMode ? "text-gray-400" : "text-gray-600"}>{recipientId}</p>
                      {remoteStream ? (
